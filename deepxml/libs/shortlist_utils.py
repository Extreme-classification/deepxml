...
def get_multiple_centroids(_ext_indices, num_centroids, features, labels):
    ...
    _cluster_obj = Cluster(
        indices=_ext_indices, embedding_dims=embedding_dims,
...
def update(data_loader, model, embedding_dim, shorty, flag=0, num_graphs=1):
    ...
    if flag == 0:
        ...
        # Multiple centroid not supported for multiple-graphs
        if num_centroids != 1 and data_loader.dataset._ext_head is not None:
